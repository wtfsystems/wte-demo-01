...
int main(int argc, char **argv) {
    ...
    auto draw_audio_opts = []() {
        ImGui::SetNextWindowPos(
            ImVec2(wte::config::gfx::screen_w / 2.0f, wte::config::gfx::screen_h / 2.0f), 0, ImVec2(0.5f, 0.5f));
        ImGui::SetNextWindowSize(ImVec2(350.0f, 250.0f));
        ImGui::Begin("Audio Settings", NULL,
            ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoCollapse);
        static float main = wte::config::volume::main;
        static float music = wte::config::volume::music;
        static float sample = wte::config::volume::sample;
        ...
        if(ImGui::Button("Cancel", ImVec2(100.0f, 30.0f))) {
            main = wte::config::volume::main;
            music = wte::config::volume::music;
            sample = wte::config::volume::sample;
        }
    ...
    auto draw_game_opts = []() {
        ...
        static bool fullscreen = ([](){
            if(wte::config::gfx::display_mode == 1) return true;
            else return false;
        }());
        static int screen_w = wte::config::gfx::screen_w;
        static int screen_h = wte::config::gfx::screen_h;
        static float scale_factor = wte::config::gfx::scale_factor;
        ...
        if(pass == 0) {
            for(std::size_t i = 0; i < wte::scale_factors.size(); i++) {
                if(wte::config::gfx::scale_factor == i) current_scale_item = i;
            }
        ...
        if(ImGui::Button("Cancel", ImVec2(100.0f, 30.0f))) {
            ...
            screen_w = wte::config::gfx::screen_w;
            screen_h = wte::config::gfx::screen_h;
            scale_factor = wte::config::gfx::scale_factor;
            for(std::size_t i = 0; i < wte::scale_factors.size(); i++) {
                if(wte::config::gfx::scale_factor > i - .001 &&
                   wte::config::gfx::scale_factor < i + .001) current_scale_item = i;
            }
            for(std::size_t i = 0; i < wte::display_modes.size(); i++) {
                if(wte::display_modes[i].width == screen_w &&
                   wte::display_modes[i].height == screen_h) {
    ...
    //  ImGui main loop
    wte::mgr::gfx::renderer::draw_gui = [draw_audio_opts, draw_game_opts](){
        //  Engine isn't running, render main menu
        if(!wte::config::flags::engine_started) {
            ...
            switch(menu_counter) {
                case 0:
                ImGui::SetNextWindowPos(
                    ImVec2(wte::config::gfx::screen_w / 2.0f, wte::config::gfx::screen_h / 2.0f),
                    0, ImVec2(0.5f, 0.5f));
        ...
        if(wte::config::flags::engine_started && wte::config::flags::engine_paused) {
            ImGui_ImplAllegro5_NewFrame();
            ImGui::NewFrame();
